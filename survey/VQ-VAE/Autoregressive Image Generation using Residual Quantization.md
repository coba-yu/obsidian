...
1.  **非条件付き画像生成 (LSUN, FFHQ)**
    -   提案手法 (RQ-VAE + RQ-Transformer)は、既存のARモデルを**上回る性能**
2.  **条件付き画像生成 (ImageNet, CC-3M)**
    -   提案手法 (RQ-VAE + RQ-Transformer)は、既存のARモデルを**上回る性能**
...
### 4. Experiments - 生成画像 - LSUN, FFHQ, CC-3M
...
*   提案手法の確率的サンプリング (stochastic sampling) も性能向上に寄与
*   大規模モデル (1.4B, 3.8Bパラメータ) では、BigGANに匹敵、あるいはそれを超える性能を達成
*   リジェクションサンプリングを用いることで、ADMを上回り、**最先端 (SOTA) のFIDスコア**を達成
